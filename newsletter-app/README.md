...
- **Recurring editorial Series:** posts keep their normal category while optionally belonging to Weekly Roundup, The Month Ahead, or Seasonal Guides. The public `GET /api/series/the-month-ahead` endpoint returns `{ item }`, where `item` is the explicitly homepage-featured published edition or the newest published edition; it returns `null` when none is available. The endpoint allows public cross-origin reads and uses a short shared cache with stale revalidation so the static homepage can fall back gracefully.
...
- **Seasonal guide:** the standalone seasonal guide is managed under `/admin/site-content`. The editor controls the active season and year, label, headline, description, destination, CTA, visibility, publication window, and an ordered selection of up to 12 published posts. Everything is stored together in `site-content.json`; `summer-guide.html` reads that file and the published article index so the guide updates without hand-editing its cards.
- **Media library:** the shared image picker stores editable names, alt text, and provider metadata in `media_assets`. When the server-only FileJump WebDAV credentials are configured, file storage and deletion use FileJump; otherwise uploads continue using the existing public Supabase bucket. Provider paths stay immutable while metadata is edited, and FileJump images are streamed through stable public media URLs so credentials never reach the browser. Deletion is blocked while an image is referenced by published or sent content.
...
2. Run every file in `supabase/migrations` in numeric order in the Supabase SQL editor. Existing installations must run `009_weekly_roundup.sql` for the shared roundup, then `010_homepage_featured_reads.sql` and `011_featured_reads_static_posts.sql` for admin-controlled Featured Reads, followed by `012_post_series.sql` for recurring Series metadata and `013_filejump_media.sql` for the shared media library, before deploying the matching application code.
...
6. Add every variable from `.env.example` to Vercel. Use long random values for `TOKEN_SECRET` and `CRON_SECRET`, and copy the webhook signing secret into `RESEND_WEBHOOK_SECRET`.
   For FileJump, store the WebDAV server, username, and app password as `FILEJUMP_WEBDAV_URL`, `FILEJUMP_USERNAME`, and `FILEJUMP_PASSWORD`. `FILEJUMP_FOLDER` scopes the editor library to a dedicated folder under the WebDAV workspace. Keep all four variables server-only.
